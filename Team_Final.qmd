...
highearn1 = df3[df3.timestamp=='2018']
highearn1 = highearn1.groupby(['title','yearsofexperience','company', 'location']).max()['yearlysalary'].reset_index()
...
highearn2 = df3[df3.timestamp=='2019']
highearn2 = highearn2.groupby(['title','yearsofexperience','company', 'location']).max()['yearlysalary'].reset_index()
...
highearn3 = df3[df3.timestamp=='2020']
highearn3 = highearn3.groupby(['title','yearsofexperience','company', 'location']).max()['yearlysalary'].reset_index()
...
highearn4 = df3[df3.timestamp=='2021']
highearn4 = highearn4.groupby(['title','yearsofexperience','company', 'location']).max()['yearlysalary'].reset_index()
...
From 2018 to 2021, those job position such as Software Engineer, Software Engineering Manager, Data Scientist, Product Manager, Hardware Engineer made highly earning. 

```{python}
# Hypothesis Testing for job position per year
# H₀: Year by year salary means of jop position are equal.
# H₁: Year by year salary means of jop position are not equal.
# α = 0.05

# Software Engineer, Software Engineering Manager, Data Scientist, Product Manager, Hardware Engineer
SE = df3.title == 
SEM = 
DS = 
PM = 
HE = 
title_ANOVA = [['Between Groups', '', '', '', '', '', ''], ['Within Groups', '', '', '', '', '', ''], ['Total', '', '', '', '', '', '']] 
ttanova_table = pd.DataFrame(title_ANOVA, columns = ['Source of Variation', 'SS', 'df', 'MS', 'F', 'P-value', 'F crit']) 
ttanova_table.set_index('Source of Variation', inplace = True)

# calculate SSTR and update anova table
x_bar = df3['yearlysalary'].mean()
SSTR = df3.groupby('title').count() * (df3.groupby('title').mean() - x_bar)**2
ttanova_table['SS']['Between Groups'] = SSTR['yearlysalary'].sum()

# calculate SSE and update anova table
SSE = (df3.groupby('title').count() - 1) * df3.groupby('title').std()**2
ttanova_table['SS']['Within Groups'] = SSE['yearlysalary'].sum()

# calculate SSTR and update anova table
SSTR = SSTR['yearlysalary'].sum() + SSE['yearlysalary'].sum()
ttanova_table['SS']['Total'] = SSTR

# update degree of freedom
ttanova_table['df']['Between Groups'] = df3['title'].nunique() - 1
ttanova_table['df']['Within Groups'] = df3.shape[0] - df3['title'].nunique()
ttanova_table['df']['Total'] = df3.shape[0] - 1

# calculate MS
ttanova_table['MS'] = ttanova_table['SS'] / ttanova_table['df']

# calculate F 
F = ttanova_table['MS']['Between Groups'] / ttanova_table['MS']['Within Groups']
ttanova_table['F']['Between Groups'] = F

# p-value
ttanova_table['P-value']['Between Groups'] = 1 - stats.f.cdf(F, ttanova_table['df']['Between Groups'], ttanova_table['df']['Within Groups'])
...
if tail_hypothesis_type == "two-tailed":
    alpha /= 2
ttanova_table['F crit']['Between Groups'] = stats.f.ppf(1-alpha, ttanova_table['df']['Between Groups'], ttanova_table['df']['Within Groups'])
...
title_conclusion = "Failed to reject the null hypothesis."
if ttanova_table['P-value']['Between Groups'] <= alpha:
    conclusion = "Null Hypothesis is rejected."
print("F-score is:", ttanova_table['F']['Between Groups'], " and p value is:", ttanova_table['P-value']['Between Groups'])    
print(title_conclusion)
