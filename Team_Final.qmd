...
df = old_df[['timestamp','company','title','location','yearsofexperience','yearsatcompany', 'gender', 'Masters_Degree', 'Bachelors_Degree', 'Doctorate_Degree', 'Highschool', 'Some_College', 'Race_Asian', 'Race_White', 'Race_Two_Or_More', 'Race_Black', 'Race_Hispanic', 'basesalary', 'stockgrantvalue', 'bonus', 'totalyearlycompensation']]
df['timestamp'] = pd.to_datetime(df['timestamp']).dt.strftime('%Y')
```
```{python}
# Check new dataset
...
```{python}
cate_loc = list(set(list(df2['location'])))
len(cate_loc) # 1050
